...
static bool node_is_empty(struct EtsMultimapNode *node) {
    assert(node != NULL);
    return IS_NULL_PTR(node->entries);
}

static struct EtsMultimapNode *ets_multimap_node_new(
                ...
                while (entry != NULL) {
                    ...
                    for (size_t j = 0; j < count; j++) {
                        if (entry == entries[j]) {
                            ...
                            ets_multimap_entry_destroy(entry, global);
                            entries[j] = NULL;
                        }
                    }
...
void ets_multimap_lookup(
    ...
    struct EtsMultimapNode *node;
    if (find_node(multimap, key, &node, global) != EtsMultimapOk) {
        return;
    }
    ...
    struct EtsMultimapEntry *entries = node->entries;

    *count = 0;
    struct EtsMultimapEntry *iter = entries;
    while (iter != NULL) {
        (*count)++;
        iter = iter->next;
    }
